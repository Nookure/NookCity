...
public final class ServerStatusManager {
  ...
  public void updateServerStatus(RegisteredServer server, boolean status) {
    if (serverStatus.containsKey(server)) {
      ...
      logger.severe(new IllegalArgumentException("Server " + server.getServerInfo().getName() + " is not registered in the server status manager."));
    }
  }

  @Nullable("If there are no servers online")
  public RegisteredServer getServerWithLessPlayers() {
  ...
  public Stream<RegisteredServer> getOnlineServers() {
    return serverStatus.entrySet().stream()
