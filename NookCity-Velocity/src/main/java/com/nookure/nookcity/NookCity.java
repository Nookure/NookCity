...
public class NookCity {
  ...
  public void onProxyInitialization(ProxyInitializeEvent event) {
    ...
    coreConfig = injector.getProvider(new Key<ConfigurationContainer<CoreConfig>>() {
    }).get();
  ...
  public void loadListeners() {
    Stream.of(PlayerWrapperListener.class)
  ...
  public void loadLobbyServers() {
    coreConfig.get().lobby.lobbyServers.forEach(serverName -> {
  ...
  private void loadTasks() {
    server.getScheduler().buildTask(
        this, injector.getInstance(ServerStatusTask.class)
    ).repeat(Duration.ofSeconds(20)).schedule();
  }
